...
from models.extractors import model_urls

class TNet(nn.Module):
    def __init__(self, backbone):
        super(TNet, self).__init__()
        if backbone in list(model_urls.keys()):
            self.backbone = PSPNet(backend=backbone, pretrained=True)
...
class MNet(nn.Module):
    ...
    def forward(self, input):
        ...
        _, us, fs = torch.split(trimap, 1, dim=1)
...
def create_shm(backbone='resnet50'):
    return SHM(backbone)
